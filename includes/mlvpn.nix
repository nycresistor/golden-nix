...
let
  ...
  pathOpts = { ... }: {
    options = {
      ...
      txRate = mkOption {
        default = 12500000;
        type = with types; int;
        description = ''The outbound transfer rate of the path. If autoRate is true, this is the max value.'';
      };
      rxRate = mkOption {
        default = 12500000;
        type = with types; int;
        description = ''The inbound transfer rate of the path. If autoRate is true, this is the max value.'';
      };
      backup = mkOption {
        default = false;
      ...
      lossLimit = mkOption {
        ...
        description = ''The max percentage of lost packets before marking the interface as down.'';
      };
    };
  };
  interfaceOpts = { ... }: {
    ...
    options = {
      keyFile = mkOption {
        example = "/private/mlvpn_key";
        type = with types; str;
        default = null;
        description = ''
          File pointing to preshared key as generated by <command>mlvpn keygen/command>.
          The shared key that mlvpn will use to connect.
        '';
      };
      bindAddress = mkOption {
        default = "0.0.0.0";
        type = with types; str;
        description = ''The address mlvpn will bind to. Currently mlvpn can only work
      ...
      remoteAddress = mkOption {
        ...
        default = null;
        description = ''The address of the remote mlvpn endpoint. Client side option only.
          If this option is set mlvpn will act as a client. If null it will be a server.'';
      };
      remotePort = mkOption {
        default = null;
        type = with types; nullOr int;
  ...
  generatePathUnit = { mlvpnInterfaceName, mlvpnInterfaceConfig, pathCfg }:
    let
      dev = mlvpnInterfaceName;
      oif = pathCfg.outboundInterfaceName;
      serviceName = pathUnitServiceName dev oif;
      preScript = ''
        SRC=$(${pkgs.iproute2}/bin/ip route get oif "${oif}" "${mlvpnInterfaceConfig.remoteAddress}" | ${pkgs.gawk}/bin/awk '/src/{getline;print $0}' RS=' ')
        if [ -z "$SRC" ]; then
          echo could not find route to remote "${mlvpnInterfaceConfig.remoteAddress}" on interface "${oif}" >&2
          exit 1
        fi
      '';
    in
    nameValuePair "mlvpn-${dev}-path-${oif}" {
      description = "mlvpn Path - ${mlvpnInterfaceName} - ${pathCfg.outboundInterfaceName}";
      requires = [ "mlvpn-${mlvpnInterfaceName}.service" ];
      after = [ "mlvpn-${mlvpnInterfaceName}.service" ];
      wantedBy = [ "multi-user.target" "mlvpn-${mlvpnInterfaceName}.service" ];
      environment.DEVICE = mlvpnInterfaceName;
      ...
      script =
        let
          ...
          setup = concatStringsSep " " (
            [ ''"${cfg.package}/bin/mlvpn" path "$SRC" dev "${mlvpnInterfaceName}" ${targetState} rate'' ]
      ...
      postStop = ''
        ${preScript}
        "${cfg.package}/bin/mlvpn" path "$SRC" dev "${mlvpnInterfaceName}" down
      '';
    };
  ...
  generateInterfaceUnit = name: values:
    let
    in
    nameValuePair "mlvpn-${name}" {
      description = "mlvpn Tunnel - ${name} ";
      ...
      script =
        let
          ...
          setup = concatStringsSep " " (
            [ ''exec ${cfg.package}/bin/mlvpn bind ${bindStr}'' ]
            ++ remoteStr
...
in
{
  options.networking.mlvpn = {
    enable = mkEnableOption "Enable mlvpn";

    package = mkOption {
      type = types.package;
      default = pkgs.mlvpn;
      defaultText = "pkgs.mlvpn";
      description = "Set version of mlvpn package to use.";
    };

    interfaces = mkOption {
      description = ''mlvpn interfaces. NOTE: This module does not configure the IP, Routes, or Firewall of the interface.
      '';
      default = { };
      example = {
        gtc-main = {
          keyFile = "/etc/glory/gtc-main.key";
        };
